    ...
    >
      <div className="relative flex items-center justify-center mb-12 w-16 h-16">
        {[0, 2].map((delay) => (
          <div
            key={delay}
            className={`absolute inset-0 rounded-full border border-[rgba(255,255,255,0.25)] bg-[rgba(255,255,255,0.03)] pointer-events-none animate-ping-auth opacity-0 ${delay === 2 ? 'delay-2000' : ''}`}
          />
        ))}
        <div 
          className="relative z-10 opacity-80 flex items-center justify-center bg-zinc-950 rounded-full animate-pulse-logo"
        >
