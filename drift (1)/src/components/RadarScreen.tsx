  ...
  return (
    <APIProvider apiKey={mapKey}>
                  ...
                  <div className="relative flex items-center justify-center w-0 h-0">
                    {[0, 1].map(delay => (
                      <motion.div
                        ...
                        className="absolute w-32 h-32 rounded-full border-[3px] border-zinc-200 bg-zinc-100/20 pointer-events-none"
                      />
                    ))}
                  </div>
                </AdvancedMarker>
      ...
      <AnimatePresence>
        {scanning && (
          <motion.div
            animate={{ scale: [0.8, 2], opacity: [0, 0.25, 0] }}
            transition={{ duration: 4, repeat: Infinity, ease: "easeOut" }}
            style={{ willChange: "transform, opacity" }}
            className={`absolute w-96 h-96 rounded-full border pointer-events-none ${hasNotes ? 'border-zinc-300 bg-zinc-300/5' : 'border-zinc-700 bg-zinc-700/5'}`}
          />
        )}
      </AnimatePresence>
